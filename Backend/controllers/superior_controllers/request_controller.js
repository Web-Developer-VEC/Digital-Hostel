...
async function profileChangeRequestSuperior(req, res) {
  try {
    ...
    const primary_gender = ["Male", "Female"];
    const requests = await requestsCollection
      .find({ gender: { $in: primary_gender } })
      .toArray();
...
async function profileUpdate(req, res) {
  try {
    ...
    const updateRequest = await tempRequestCollection.findOne({
      registration_number,
    });
    if (!updateRequest) {
    ...
    const hasFoodTypeChange = changesArray.some((change) =>
      /^food_type: /.test(change),
    );
    if (action === "approve") {
...
async function confirmVacate(req, res) {
  try {
    ...
    if (action === "approve") {
      ...
      return res.json({
        message:
          "Student archived and removed from student database & vacate forms",
      });
    }
