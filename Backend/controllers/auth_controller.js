...
async function sendOTP(req, res) {
  try {
    ...
    await sendOTPForForgetPassword(
      warden_data.phone_number,
      warden_data.warden_name,
      req,
    );
...
async function setNewPassword(req, res) {
  try {
    ...
    if (updateResult.modifiedCount === 0) {
      return res.status(400).json({
        error:
          "Failed to update password. Warden not found or same password used.",
      });
    }
...
async function DeleteWarden_Student(req, res) {
  ...
  try {
    ...
    if (type === "student") {
      ...
      result = await wardenCollection.deleteOne({
        unique_id: registration_number,
      });
    } else {
      return res
        .status(400)
        .json({ error: "Invalid type. Must be 'student' or 'warden'." });
    }

    if (result.deletedCount === 0) {
      return res
        .status(404)
        .json({ error: "No record found with the given registration number." });
    }

    return res
      .status(200)
      .json({ message: `${type} record deleted successfully.` });
  } catch (error) {
