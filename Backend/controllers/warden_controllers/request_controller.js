  ...
  try {
    ...
    const hasOtherChanges = changesArray.some((change) =>
      [
        ...
        "phone_number_parent:",
      ].some((key) => change.includes(key)),
    );
    let editStatus = hasOtherChanges
      ? null
      : action === "approve"
        ? true
        : false;
    ...
    return res.status(200).json({
      ...
      newFoodType:
        action === "approve" ? request.requested_foodtype : undefined,
      effectiveDate:
        action === "approve"
          ? updateTimeIST.toLocaleString("en-IN", { timeZone: "Asia/Kolkata" })
          : undefined,
    });
  } catch (error) {
