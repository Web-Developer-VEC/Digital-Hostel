...
async function markStudentVacate (req, res) {
    try {
        ...
        return res.status(200).json({ message: "Student marked for vacating successfully" });
    } catch (err) {
...
async function foodChangeDirect (req, res) {
    ...
    try {
        ...
        return res.status(200).json({ message: `Food type updated to ${newFoodType}`, foodtype: newFoodType });
    } catch (err) {
        console.error("❌ Error:", err);
       return res.status(500).json({ error: "Internal Server error" });
    }
...
async function roomnoChangeDirect(req, res) {
    try {
        ...
        return res.status(200).json({ message: "Room number updated successfully" });
    } catch (error) {
