...
cron.schedule("0 * * * *", async () => {
  ...
  try {
    ...
    const pendingUpdates = await cronCollection
      .find({ updation_time: { $lte: now } })
      .toArray();
    ...
    for (const update of pendingUpdates) {
      ...
      if (result.modifiedCount > 0) {
        console.log(
          `Successfully updated food type for ${name} (${registration_number}) to ${foodtype}.`,
        );
      } else {
        console.log(
          `No matching student found for ${name} (${registration_number}).`,
        );
      }
